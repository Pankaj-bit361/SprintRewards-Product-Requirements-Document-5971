...
const { FiMail, FiLock, FiLogIn, FiUser, FiShield, FiAward, FiTrendingUp, FiWifi, FiWifiOff } = FiIcons;
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        if (result.isOffline) {
          toast.success('Welcome to SprintRewards! (Offline Mode)', {
            icon: '🔌',
            style: {
  ...
  const featureCards = [
    {
      ...
      description: 'Earn points through performance and unlock rewards'
    },
    {
      icon: FiTrendingUp,
      title: 'Track Progress',
      description: 'Monitor your sprint achievements and growth'
    },
    {
      icon: FiShield,
      title: 'AI Validation',
      description: 'Fair assessment powered by intelligent algorithms'
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black overflow-hidden">
      ...
      {isOffline && (
        ...
        >
          <div className="glass-card px-4 py-2 rounded-full border border-yellow-500/30 bg-yellow-500/10">
            <div className="flex items-center space-x-2">
              <SafeIcon icon={FiWifiOff} className="w-4 h-4 text-yellow-400" />
              ...
              >
                ...
                <div className="text-center mb-8">
                  ...
                  {isOffline && (
                    <div className="mt-2 flex items-center justify-center space-x-1">
                      <SafeIcon icon={FiWifiOff} className="w-4 h-4 text-yellow-400" />
                      <span className="text-yellow-400 text-sm">Offline Mode Available</span>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-6 text-center">
                  <p className="text-xs text-gray-500">
                    {isOffline ? '🔌 Working in offline mode' : 'Protected by AI-powered authentication'}
                  </p>
                </div>
              </motion.div>
