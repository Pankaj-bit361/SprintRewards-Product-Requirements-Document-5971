...
const {FiSend,FiGift,FiArrowRight,FiArrowLeft}=FiIcons;
const Transactions=()=> {
...
setEmployees(usersRes.data.filter(u=> u._id !==user._id));
} catch (error) {
...
if (pointsToSend > user.rewardPoints) {
toast.error('Insufficient reward points');
return;
}
try {
await api.post('/transactions/send',{...sendForm,points: pointsToSend});
...
className="bg-gray-900 border border-gray-800 rounded-xl p-6 text-white mb-8"
>
<div className="flex items-center justify-between">
<div>
<p className="text-gray-400 mb-1">Available Balance</p>
<p className="text-3xl font-bold text-white">{user?.rewardPoints} Points</p>
</div>
...
<div className={`w-12 h-12 rounded-full flex items-center justify-center ${isSent ? 'bg-gray-700' : 'bg-gray-700'}`}>
<SafeIcon icon={isSent ? FiArrowRight : FiArrowLeft} className={`w-6 h-6 text-white`} />
</div>
<div>
<p className="font-medium text-white">{isSent ? 'Sent to' : 'Received from'} {otherUser.name}</p>
<p className="text-sm text-gray-500">{new Date(transaction.createdAt).toLocaleDateString('en-US',{
year: 'numeric',
month: 'short',
day: 'numeric',
hour: '2-digit',
...
<div>
<label className="block text-sm font-medium text-gray-300 mb-2">Points (Available: {user?.rewardPoints})</label>
<input
type="number"
min="1"
