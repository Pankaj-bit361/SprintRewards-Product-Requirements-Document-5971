...
const { FiSend, FiGift, FiUsers, FiArrowRight, FiArrowLeft, FiPlus } = FiIcons;
...
const Transactions = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setEmployees(usersRes.data.filter(u => u._id !== user.id));
    } catch (error) {
  ...
  const handleSendPoints = async (e) => {
    e.preventDefault();
    if (parseInt(sendForm.points) > user.rewardPoints) {
      toast.error('Insufficient reward points');
      return;
    }
    try {
      await axios.post('/transactions/send', sendForm);
      toast.success('Points sent successfully!');
      // Update user balance
      updateUser({
        rewardPoints: user.rewardPoints - parseInt(sendForm.points),
        totalGiven: user.totalGiven + parseInt(sendForm.points)
      });
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                        ...
                        <SafeIcon
                          icon={isSent ? FiArrowRight : FiArrowLeft}
                      ...
                      <div>
                        ...
                        {transaction.message && (
                          <p className="text-sm text-gray-400 italic mt-1">"{transaction.message}"</p>
                        )}
                      </div>
                    </div>
                    <div className="text-right">
                      <span className={`text-lg font-bold ${isSent ? 'text-gray-400' : 'text-white'}`}>
            ...
            <form onSubmit={handleSendPoints} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">
                  Points (Available: {user?.rewardPoints})
                </label>
                <input
                  type="number"
                  min="1"
