...
const { FiTrendingUp, FiGift, FiUsers, FiClock, FiCheckCircle, FiAlertCircle, FiUnlock, FiWifi, FiWifiOff, FiActivity, FiZap, FiTarget, FiCpu, FiDatabase } = FiIcons;
const { HiSparkles, HiCube, HiLightningBolt, HiRocketLaunch } = HiIcons;
const { AiOutlineDashboard, AiOutlineTrophy, AiOutlineFire } = AiIcons;
const { BsGraphUp, BsShieldCheck, BsStars } = BsIcons;

const Dashboard = () => {
  const { user, updateUser, isOffline } = useAuth();
  const [stats, setStats] = useState({
    currentSprint: null,
    recentTransactions: [],
    leaderboard: { topGivers: [], topReceivers: [] }
  });
      ...
      if (isOffline) {
        const mockData = {
          ...
          recentTransactions: [
            {
              _id: '1',
              fromUserId: { _id: '2', name: 'Alice Smith' },
              toUserId: { _id: user?.id, name: user?.name },
              points: 50,
            ...
            {
              _id: '2',
              fromUserId: { _id: user?.id, name: user?.name },
              toUserId: { _id: '3', name: 'Bob Johnson' },
              points: 25,
          ...
          leaderboard: {
            topGivers: [
              { _id: '1', name: 'Alice Smith', totalGiven: 300 },
              { _id: '2', name: 'Bob Johnson', totalGiven: 250 },
              { _id: user?.id, name: user?.name, totalGiven: user?.totalGiven || 0 }
            ],
            topReceivers: [
              { _id: '3', name: 'Carol White', totalReceived: 280 },
              { _id: '4', name: 'David Brown', totalReceived: 220 },
              { _id: user?.id, name: user?.name, totalReceived: user?.totalReceived || 0 }
            ]
          }
        };
        setStats(mockData);
      } else {
        const response = await fetch('http://localhost:3001/api/sprints/current', {
          headers: {
        ...
        if (response.ok) {
          ...
          setStats(prev => ({ ...prev, currentSprint: sprintData }));
        }
      }
    } catch (error) {
  ...
  const handleUnlockPoints = async () => {
    if (isOffline) {
      updateUser({ 
        rewardPoints: (user?.rewardPoints || 0) + 500, 
        unlockedThisSprint: true 
      });
      toast.success('Successfully unlocked 500 reward points! (Offline Mode)');
      return;
    }

    try {
      const response = await fetch('http://localhost:3001/api/users/unlock-points', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      ...
      if (response.ok) {
        ...
        updateUser({ rewardPoints: data.rewardPoints, unlockedThisSprint: true });
      } else {
        const error = await response.json();
        toast.error(error.message);
      }
    } catch (error) {
      toast.error('Failed to unlock points');
    }
  };
                  ...
                  <p className="text-gray-400 text-lg">
                    ...
                    {isOffline && (
                      <span className="ml-2 text-gray-400">
                        <SafeIcon icon={FiWifiOff} className="inline w-4 h-4 mr-1" />
                        Offline Mode
                      </span>
                    )}
                  </p>
                            ...
                            <div>
                              <div className="text-sm font-medium text-white">
                                ...
                                <span className="ml-1 text-white">
                                  {isSent ? transaction.toUserId.name : transaction.fromUserId.name}
                                </span>
                              </div>
        ...
        >
          <div className="bg-gray-900 border border-gray-800 rounded-3xl p-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { icon: FiGift, label: 'Send Points' },
                { icon: FiUsers, label: 'Leaderboard' },
                { icon: FiTarget, label: 'Tasks' },
                ...
                <motion.button
                  ...
                  whileHover={{ scale: 1.05, y: -5 }}
                  whileTap={{ scale: 0.95 }}
                  className={`p-6 rounded-2xl bg-gray-800 border border-gray-700 hover:border-gray-600 transition-all duration-300 group`}
                >
                  <SafeIcon icon={action.icon} className={`w-8 h-8 mb-3 text-white group-hover:scale-110 transition-transform`} />
