...
const { FiUser, FiLogOut, FiMenu, FiX, FiTrendingUp, FiUsers, FiSettings, FiList, FiWifi, FiWifiOff } = FiIcons;

const Navbar = () => {
  const { user, logout, isFounder, isOffline } = useAuth();
        ...
        <div className="bg-gray-900 border-b border-gray-800">
          <div className="max-w-7xl mx-auto px-4 py-1">
            <div className="flex items-center justify-center space-x-2">
              ...
              <span className="text-gray-400 text-sm">Offline Mode - Limited Functionality</span>
            </div>
          </div>
        </div>
      )}
          ...
          <Link to="/" className="flex items-center space-x-2">
            ...
            {isOffline && (
              <div className="flex items-center space-x-1 ml-2">
                <SafeIcon icon={FiWifiOff} className="w-3 h-3 text-gray-400" />
              </div>
            )}
          </Link>
        ...
        >
          <div className="px-2 pt-2 pb-3 space-y-1">
            ...
            <div className="border-t border-gray-900 pt-2">
              ...
              <button
                onClick={handleLogout}
                className="flex items-center space-x-2 w-full px-3 py-2 text-left text-gray-400 hover:text-white hover:bg-gray-900 rounded-md"
              >
                <SafeIcon icon={FiLogOut} className="w-5 h-5" />
                <span>Logout</span>
              </button>
